...
class _SfxSpec {
  ...
  /// 재생이 끝난 뒤 이 사운드가 붙들고 있을 플레이어의 수.
  ///
  /// 겹쳐 울릴 수 있는 수를 막지는 않는다 — [AudioPool] 은 여유 플레이어가
  /// 없으면 그때그때 새로 만들어 낸다. 자주 겹치는 소리일수록 크게 잡아 두면
  /// 그 새로 만드는 일이 줄어든다.
...
class GameAudio {
  ...
  static const Map<Sfx, _SfxSpec> _specs = {
    ...
    // ── 이동 ─────────────────────────────────────────────────────
    // 대시 소리는 재사용 대기(0.9초)보다 짧아야 한다. 길면 연타할 때 앞선
    // 소리 위에 새 소리가 얹혀 진폭이 더해진다 — 누를수록 커진다.
    //
    // 세기는 회피 동작에 걸맞은 자리에 둔다. 파일 자체의 세기(가장 센 100ms
    // 구간 RMS 0.34)에 이 값을 곱한 것이 체감 세기다. 0.40 이면 근접 명중
    // (0.12)과 폭발(0.22) 사이 — 눌렀다는 것은 분명히 들리되, 전투음을
    // 밀어내지는 않는다.
    Sfx.dash: _SfxSpec(
      ['dash_0', 'dash_1'],
      volume: 0.40,
  ...
  static void play(Sfx sfx, {Vector2? at, double volumeScale = 1.0}) {
    ...
    //
    // 울리고 있는 소리를 이쪽에서 끊지는 않는다. [AudioPool] 이 돌려주는
    // 정지 함수는 플레이어를 식별자로 붙잡는데, 재생이 끝난 플레이어는 풀로
    // 돌아가 다음 소리에 다시 쓰인다. 그래서 예전 소리의 정지 함수를 뒤늦게
    // 부르면 방금 시작한 다른 소리를 끄게 된다. 소리가 겹쳐 쌓이는 것은 파일
    // 길이를 다시 울리는 간격보다 짧게 잡아서 막는다.
