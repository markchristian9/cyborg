...
## 11. 실사용 가능하게 (2026-08-05 · "make it better")

### 11.1 차단지점이 스스로 풀렸다

§9 가 보류 사유로 든 두 가지가 다른 세션의 진척으로 해소됐다.

| §9 의 보류 사유 | 지금 |
|---|---|
| Dart 바인딩에 `party` 표가 없어 구현체를 못 만든다 | ✅ `generated/party.dart`·`party_member.dart`·`party_invite.dart` 생성됨. 서버도 배포된 것으로 보인다 |
| `attack_monster` 를 클라가 부르지 않아 "함께 사냥" 이 성립하지 않는다 | ✅ `spacetime_world_presence.dart:169` 에서 호출한다. §4 쟁점 B 의 한계가 사라졌다 |

**§1 의 핵심 판정은 여전히 유효하다.** 서버에 `monster_ai_timer`·`regen_timer` 가 생겨 주기 작업이 늘었지만, 이동은 그대로 `move_to`(클라 보고 + 속도 상한, `world.rs:1040-1044`)다. follower 를 매 프레임 밀어 줄 자리가 서버에 여전히 없으므로 추종은 클라이언트가 맡는다.

### 11.2 만든 것

- **`lib/spacetime/spacetime_party.dart`** — `SpacetimeWorldPresence` 와 같은 모양(구독 생애·세대 번호). 뷰 셋(`my_party`·`my_party_members`·`my_party_invites`)을 읽고 reducer 아홉을 부른다.
- **`kPartySubscriptions`** — 월드 구독과 **한 배열로 합치지 않았다.** 파티는 공개 표가 아니라 view 이고, 한 곳에 두면 월드와 파티를 각각 손보는 사람이 같은 줄에서 부딪친다.
- **`lib/game/ui/party_panel.dart`** — 파티 패널과 초대 카드.
- **`test/party_session_test.dart`** — 파생 규칙 14 건.

### 11.3 설계 판단 셋

**초대의 진입점을 목록으로 만들었다.** 원격 요원을 눌러 초대하는 길이 이 게임에는 없다 — 탭은 땅으로만 간다(`click_move.dart`). 그 길을 새로 뚫으면 팀 A 가 만지는 입력·렌더 코드를 건드려야 한다. 그래서 파티가 없을 때의 패널을 **근처 요원 목록**(40 타일 안)으로 만들어 거기서 고르게 했다. 충돌 없이 파티를 맺을 수 있고, 목록은 어차피 필요한 화면이다.

**초대 카드를 패널과 분리했다.** 초대는 20 초 만에 사라지므로 메뉴를 열어야 보이는 자리에 두면 대부분 놓친다.

**거절 사유를 삼키지 않는다.** "상대가 이미 다른 파티에 있다" 처럼 서버가 문장으로 주는 사유를 지우면 눌렀는데 아무 일도 없는 화면이 된다. `cleanReducerError`(기존 인증 화면과 같은 경로)로 다듬어 배너에 띄운다. 좌표 보고와 다른 점이 이것이다 — 그쪽은 다음 주기가 따라잡지만 초대는 그렇지 않다.

### 11.4 검증

`cargo test` 57 · `flutter test` 224(파티 29) · `flutter analyze` 파티 코드 무결.

### 11.5 커밋 범위 — 이번엔 통합했다

`a098971` 에 다른 세션의 진행 중 변경이 함께 들어갔다. 파티 UI 는 게임 배선 없이 컴파일되지 않는데 그 배선(`action_rpg_game.dart`·`main.dart`·`cyborg_connection.dart`)이 팀 A 의 미커밋 작업과 같은 파일에 얽혀 있다. 부분 커밋은 **검증되지 않은 조합**을 만들므로, 위 검증을 통과한 트리 상태를 그대로 담는 편이 안전하다고 판단했다.

### 11.6 아직 남은 것

- **실서버 2계정 검증** — 코드만으로 되는 경로가 `test/world_presence_test.dart:22-60` 에 있다. 그 패턴으로 초대→수락→추종을 확인해야 "동작한다" 고 말할 수 있다. 지금까지의 검증은 단위와 정적 분석까지다.
- **추방 버튼** — 서버 `kick_member` 와 클라 `kick()` 은 있으나 패널에서 부르지 않는다. 파티장이 누르는 자리는 지금 위임에 쓰고 있어, 한 줄에 두 동작을 담으려면 눌렀을 때 고르는 단계가 필요하다.
- **파티원 표식** — 월드 화면에서 파티원을 구분해 주지 않는다. `remote_player.dart` 가 팀 A 소유라 별도 오버레이로 얹어야 한다(§5 의 권고).
